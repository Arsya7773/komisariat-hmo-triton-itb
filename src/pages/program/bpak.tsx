...
import { bpakData } from '../../data/bpData';
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <div className="flex justify-center w-full mt-4">
            <div className="relative inline-block text-left text-yellow-400 text-xl md:text-2xl uppercase tracking-[0.4em] font-black drop-shadow-[0_0_15px_rgba(250,204,21,0.6)]">
              {/* FIX: Tambahkan whitespace-nowrap */}
        ...
        >
          ...
          <div className="mt-16 space-y-16 bg-[#112240]/40 border border-white/5 p-8 md:p-12 rounded-[3rem] shadow-inner backdrop-blur-sm relative z-10">
            <h3 className="text-3xl font-bold text-cyan-50 border-l-4 border-cyan-400 Handlers pl-5 mb-6">Komisi & Implementasi Fungsi</h3>
            ...
            {bpakData.commissions && bpakData.commissions.map((komisi: any) => (
              <div key={komisi.id}>
                <div className="flex flex-col lg:flex-row items-center justify-between mb-10 gap-6 border-b border-cyan-900/50 pb-8">
                  <h3 className="text-3xl font-bold text-cyan-50 border-l-4 border-cyan-400 pl-5">{komisi.title}</h3>
                  <GalaxyCapsule className="p-2 pr-8 rounded-full flex items-center gap-4 w-fit max-w-full">
                    <div className="w-14 h-14 rounded-full bg-[#071026] flex items-center justify-center text-xl border-2 border-cyan-400 relative z-10">⚙️</div>
                    <div className="text-left relative z-10 pl-1">
                      <h4 className="font-bold text-white tracking-wide">Implementasi</h4>
                      <p className="text-cyan-300/90 text-xs font-medium uppercase tracking-wider">{komisi.desc}</p>
                    </div>
                  </GalaxyCapsule>
                </div>

                {komisi.functions && komisi.functions.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
                    {komisi.functions.map((func: any) => (
                      <AnimatedProgramCard key={func.id} program={func} />
                    ))}
                  </div>
                )}
