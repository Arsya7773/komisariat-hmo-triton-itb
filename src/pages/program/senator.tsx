...
import { senatorData } from '../../data/senatorData';
    ...
    <div className="bg-[#0A192F] min-h-screen pb-32 text-white relative overflow-hidden font-sans pt-32">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <motion.header initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-20">
          <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-4 text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-white to-cyan-300 drop-shadow-[0_0_30px_rgba(34,211,238,0.5)]">
            Senator Komisariat
          </h1>
          <p className="text-yellow-400 text-xl md:text-2xl uppercase tracking-[0.4em] font-black">
            Penghubung Massa & Pusat
          </p>
        </motion.header>

        <section className="mb-20 flex flex-col items-center">
          <GalaxyCapsule className="p-2 pr-10 rounded-full max-w-full mb-6">
            <img src={senatorData.leader.photo} alt={senatorData.leader.name} loading="eager" width={64} height={64} className="w-16 h-16 rounded-full object-cover border-2 border-yellow-400 relative z-10 bg-[#0A192F]" onError={(e) => { (e.currentTarget as HTMLImageElement).src = fallbackImage; }} />
            <div className="text-left relative z-10 pl-2">
              <h3 className="font-bold text-white text-lg">{senatorData.leader.name}</h3>
              <p className="text-yellow-400 text-xs font-bold uppercase tracking-widest mt-1">{senatorData.leader.role}</p>
            </div>
          </GalaxyCapsule>
        </section>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {senatorData.workloads.map((work) => (
            <AnimatedProgramCard key={work.id} program={work} />
          ))}
        </div>

        {senatorData.bapaPrograms.length > 0 && (
          <div className="mt-20">
            <h3 className="text-2xl font-bold mb-10 border-l-4 border-yellow-400 pl-4">Program & Aspirasi BAPA</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {senatorData.bapaPrograms.map((bapa) => (
                <AnimatedProgramCard key={bapa.id} program={bapa} />
              ))}
            </div>
          </div>
        )}
